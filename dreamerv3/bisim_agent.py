...
class BisimAgent(embodied.jax.Agent):
  ...
  def bisim_loss(self, carry, obs, prevact, training=True):
    ...
    dyn_carry, dyn_entries, repfeat = self.dyn.observe(dyn_carry, tokens, prevact, reset, training)
    z = repfeat['logit']
    ...
    z_dist = jnp.mean(jnp.abs(z- z2), axis=-1)   # per-dimension distance
